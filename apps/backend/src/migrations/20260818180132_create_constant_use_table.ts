...
export async function up(db: Kysely<Database>): Promise<void> {
    await db.schema.withSchema('municipal_qc')
        ...
        .addColumn('const_id','integer',ref=>ref.references('common.constants_table.const_id').onDelete('cascade'))
        .addColumn('eq_id','integer',ref=>ref.references('municipal_qc.eqs_table.eq_id').onDelete('cascade'))
        .execute()
}
